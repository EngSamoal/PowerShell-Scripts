...
# 0. Banner
# =====================================================================================
Write-Host "Invoke-RemoteSecurityGapRemediation.ps1" -ForegroundColor Magenta
if ($Apply) {
...
Write-Log "Run started. Apply=$Apply. VMs=$($vmNames.Count). vCenter(s)=$(($connectedServers | ForEach-Object { $_.Name }) -join ', ')."
...
# Runs a large PowerShell payload inside the guest WITHOUT hitting Invoke-VMScript's
# effective command-line length limit. Invoke-VMScript -ScriptType Powershell encodes
# the whole script onto a command line; a multi-KB payload overflows CreateProcess and
# fails with the misleading "Could not locate Powershell script interpreter ... you do
# not have enough permissions" error. Instead we:
#   1. Base64 the payload and write it into a guest temp file in small chunks
#      (each Invoke-VMScript call stays a few KB - always safe).
#   2. Run one short bootstrapper that decodes the file and executes it in a child
#      powershell.exe (-ExecutionPolicy Bypass), returning its stdout (the envelope).
#   3. Always delete both guest temp files, success or failure.
# Everything goes through Invoke-VMScript only - no Copy-VMGuestFile, no WinRM, no
# ESXi/guest network path. The temp file contains only the security-check logic plus
# the injected boolean switches; it holds no credential.
function Invoke-LargeGuestPayload {
    ...
    $guestPath  = 'C:\Windows\Temp\SecGap_' + ([guid]::NewGuid().ToString('N')) + '.b64'
    $decodePath = 'C:\Windows\Temp\SecGap_' + ([guid]::NewGuid().ToString('N')) + '.run.ps1'
    ...
    try {
        for ($i = 0; $i -lt $total; $i++) {
            ...
            $st = "$verb -LiteralPath '$guestPath' -Value '$part' -Encoding Ascii -NoNewline"
...
`$ErrorActionPreference = 'Stop'
try {
    `$b64 = Get-Content -LiteralPath '$guestPath' -Raw
    `$txt = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String(`$b64))
    Set-Content -LiteralPath '$decodePath' -Value `$txt -Encoding UTF8
} catch {
    Write-Output ('<<<STAGE-ERROR>>>' + `$_.Exception.Message + '<<<END-STAGE-ERROR>>>')
    return
}
& powershell.exe -NonInteractive -NoProfile -ExecutionPolicy Bypass -File '$decodePath' 2>&1
"@
        ...
        $cleanup = "Remove-Item -LiteralPath '$guestPath','$decodePath' -Force -ErrorAction SilentlyContinue"
        try {
...
foreach ($vmName in $vmNames) {
    ...
    try {
        ...
        foreach ($r in @($envelope.Rows)) {
            ...
            if ($r.RequiresReboot -and $disp.Status -in 'Remediated', 'Non-Compliant') { $f.Reboot = $true }
...
if (-not $Apply) {
    Write-Host ""
    Write-Host "This was a DRY RUN. Re-run with -Apply (and any -Disable*/-Enforce* opt-ins) to remediate." -ForegroundColor Yellow
}
