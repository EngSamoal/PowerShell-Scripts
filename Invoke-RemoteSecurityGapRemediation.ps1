...
$ScriptBuild = '2026-08-28i-winrm-localsubnet'
...
Write-Host ("If the build above is not '2026-08-28i-winrm-localsubnet' you are running an OLD copy - update it.") -ForegroundColor DarkGray
if ($Apply) {
...
try {
    ...
    if (-not $SkipCredentialGuard) {
        ...
        $miiDomainJoined = $null
        try { $miiDomainJoined = [bool](Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction Stop).PartOfDomain } catch { $miiDomainJoined = $null }
        if ($miiDomainJoined -eq $false) {
            Add-ResultRow -Category 'VBS & Credential Protection' -Item 'Enable Machine Identity Isolation' -Status 'Not Applicable' `
                -Details 'This host is not domain-joined (workgroup). Machine Identity Isolation binds the Active Directory computer-account credential (domain secure-channel / Netlogon secret) into the VBS/IUM enclave; a workgroup server has no AD machine identity to isolate, so the DeviceGuard\MachineIdentityIsolation policy is a no-op that does not persist. Enable it only after the server is domain-joined (it also requires VBS running + a reboot).' `
                -DetectedValue 'PartOfDomain=False' -ExpectedValue 'N/A unless domain-joined'
        } else {
            $p = @{
