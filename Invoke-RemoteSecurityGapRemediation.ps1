...
foreach ($vmName in $vmNames) {
    ...
    try {
        ...
        Write-Log "'$vmName': invoking in-guest payload (Apply=$effectiveApply, user='$($GuestCredential.UserName)')." 'INFO'

        $run = Invoke-VMScript -VM $vm -Server $srv -GuestCredential $GuestCredential -ScriptText $payload `
                               -ScriptType Powershell -ToolsWaitSecs $ToolsWaitSecs -Confirm:$false -ErrorAction Stop
