...
# See the identical helper in Invoke-RemoteSecurityGapRemediation.ps1 for the rationale:
# Invoke-VMScript -ScriptType Powershell encodes the whole script onto a command line,
# so a multi-KB payload overflows CreateProcess and fails with a misleading
# "Could not locate Powershell script interpreter" error. This stages the payload into
# a guest temp file in small chunks, runs a short bootstrapper, and always cleans up.
# Uses Invoke-VMScript only - no Copy-VMGuestFile, no WinRM, no guest network path.
