...
param(
    ...
    # Force every target to be treated as workgroup / non-domain: Kerberos is then
    # reported "Not Applicable" (no AD domain / KDC) without relying on in-guest
    # DomainRole detection.
...
Write-Host "Invoke-RemoteKerberosCheck.ps1 - event-based Kerberos encryption evidence (read-only)  [build 2026-08-28j-workgroup-na]" -ForegroundColor Magenta
...
$isDC = $role -in 4, 5
# Standalone / workgroup (role 0 or 2), or forced by the operator: no AD domain,
# no KDC - Kerberos is not used at all, so the whole control is Not Applicable.
$isWorkgroup  = $TreatAllAsWorkgroup -or ($role -in 0, 2)
$workgroupWhy = if ($TreatAllAsWorkgroup) { '-TreatAllAsWorkgroup' } else { "DomainRole=$role ($roleName)" }
...
if ($isWorkgroup) {
    $rows.Add([pscustomobject]@{
        SecurityCheck  = 'Kerberos encryption (Event 4768) - scope'
        EncTypeCode    = ''
        EventCount     = $eventsReturned
        NewestSample   = ''
        SampleAccounts = ''
        Status         = 'Not Applicable'
        Detail         = "NOT APPLICABLE - this server is in a workgroup ($workgroupWhy). There is no Active Directory domain or KDC; authentication is NTLM / local accounts only, so Kerberos (and its AES / SHA256/384/512 encryption types) is not used. Nothing to assess or remediate."
    }) | Out-Null
    $overall = 'Not Applicable'
    $overallDetail = "Workgroup server ($workgroupWhy) - no AD domain / KDC; Kerberos is not in use."
} elseif (-not $isDC) {
    $rows.Add([pscustomobject]@{
        ...
        Detail         = "DomainRole=$role ($roleName). Event ID 4768 (Kerberos TGT / AS-REQ) is generated by the KDC and is therefore expected only on Domain Controllers. This host is a domain member but not a DC, so an absence of 4768 events here is expected and is NOT a compliance failure. Run this check against your Domain Controllers to assess Kerberos ticket encryption (Event 4769 on the DCs shows what was negotiated for member servers). 4768 events actually present on this host: $eventsReturned."
    }) | Out-Null
    $overall = 'Manual Verification Required'
    $overallDetail = "Domain member, not a Domain Controller (role $role / $roleName) - 4768 ticket-encryption evidence is only meaningful on a KDC."
...
# Detailed on-screen printout for one VM - one line per row plus detected/expected
# value and reason as indented sub-lines.
function Write-VmDetail {
    ...
    foreach ($row in $Rows) {
        ...
        Write-Host ("  [{0}] " -f $status.PadRight(28)) -ForegroundColor $color -NoNewline
        Write-Host ("{0}" -f $row.SecurityCheck)
        $dv = (([string]$row.DetectedValue) -replace '\s+', ' ').Trim()
        $ev = (([string]$row.ExpectedValue) -replace '\s+', ' ').Trim()
        if ($dv) { Write-Host ("        detected : {0}" -f $dv) -ForegroundColor DarkGray }
        if ($ev) { Write-Host ("        expected : {0}" -f $ev) -ForegroundColor DarkGray }
        $rsn = (([string]$row.'Error/Reason') -replace '\s+', ' ').Trim()
        if ($rsn) {
            if ($rsn.Length -gt 500) { $rsn = $rsn.Substring(0, 500) + '...' }
            Write-Host ("        detail   : {0}" -f $rsn) -ForegroundColor DarkGray
        }
    }
}
...
$guestPayload = $PayloadTemplate.Replace('{{MAXEVENTS}}', [string][int]$MaxEvents).Replace('{{TREAT_ALL_AS_WORKGROUP}}', $(if ($TreatAllAsWorkgroup) { '$true' } else { '$false' }))
...
foreach ($vmName in $vmNames) {
    ...
    try {
        ...
        # Detailed on-screen result for this VM (in addition to the CSV).
        Write-VmDetail -Header "$vmName  @ $($srv.Name)   ($ghost / $gip)  -  $roleNote" -Rows $centralRows.GetRange($vmRowStart, $centralRows.Count - $vmRowStart)
        Write-Host ("  OVERALL: {0}" -f $meta.Overall) -ForegroundColor $(switch ($meta.Overall) { 'Compliant' { 'Green' } 'Non-Compliant' { 'Red' } 'Manual Verification Required' { 'DarkYellow' } 'Not Applicable' { 'Gray' } default { 'Yellow' } })
        if ($meta.OverallDetail) { Write-Host ("           {0}" -f (([string]$meta.OverallDetail) -replace '\s+', ' ')) -ForegroundColor DarkGray }
...
Write-Host ("  Not Applicable (workgroup / no AD) : {0}" -f $summary.NotApplicable) -ForegroundColor Gray
...
Write-Host "Note   : Workgroup servers -> 'Not Applicable' (no AD domain / KDC; Kerberos not used). Domain member non-DC -> 'Manual Verification Required' (check the DCs; Event 4768 is KDC-only)." -ForegroundColor DarkYellow
