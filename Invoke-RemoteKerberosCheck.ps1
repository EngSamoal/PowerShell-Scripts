...
foreach ($vmName in $vmNames) {
    ...
    try {
        ...
        Write-Log "'$vmName': running Kerberos 4768 query payload." 'INFO'
        $run = Invoke-VMScript -VM $vm -Server $srv -GuestCredential $GuestCredential -ScriptText $guestPayload `
                               -ScriptType Powershell -ToolsWaitSecs $ToolsWaitSecs -Confirm:$false -ErrorAction Stop

        $envelope = Read-EnvelopeFromScriptOutput -Output $run.ScriptOutput -StartMarker '<<<KRB-ENVELOPE-B64>>>' -EndMarker '<<<END-KRB-ENVELOPE>>>'
        if ($null -eq $envelope) {
            $summary.SkippedFailed++
