...
.DESCRIPTION
    ...
    Matching logic:
      Azure Arc identifies a machine by its guest OS hostname, which is not
      always the same string as the vCenter display name (VM Name). To bridge
      that gap, each vCenter VM is matched against Arc using TWO candidate
      identifiers, checked in this order:
        a) VM Name (the vCenter display name)
        b) DNS Name (the guest OS hostname reported by VMware Tools), but
           ONLY when it is non-blank, not a generic placeholder value
           (localhost, localhost.localdomain, etc.), and UNIQUE across the
           vCenter list. A DNS Name shared by multiple VMs (e.g. several VMs
           all reporting "localhost") is never used as a match key, since it
           could not reliably identify a single VM and would risk associating
           the wrong machine with an Arc record.

      1. Exact match  : a candidate identifier, trimmed of leading/trailing
                         whitespace, compared CASE-INSENSITIVELY against Arc's
                         VM Name (e.g. 'tb-crbl-prx01' = 'TB-CRBL-PRX01' - this
                         is a display-convention difference, not a naming
                         ambiguity: Linux hosts commonly report a lowercase
                         hostname while the vCenter display name was set in
                         uppercase). Only an exact match on VM Name OR DNS Name
                         sets ArcOnboardingStatus = 'Onboarded'.
      2. Possible match: Only evaluated for candidate identifiers that did NOT
                         exact-match. Names are normalized (uppercased,
                         hyphens/underscores/spaces stripped) into a "loose
                         key". If the loose keys match, the VM is flagged as a
                         high-confidence candidate for manual review only - it
                         never sets Onboarded. This bucket only catches genuine
                         structural differences: hyphen vs underscore,
                         missing/extra hyphen, accidental spaces - since pure
                         case differences are handled by the exact match above.
...
.PARAMETER VCenterNameColumn
    Name of the VM Name (display name) column in List A. Default: 'VM Name'.

.PARAMETER VCenterDnsNameColumn
    Name of the guest OS hostname / DNS Name column in List A, used as a
    fallback match key when the VM Name doesn't match Arc's hostname-based
    name. Default: 'DNS Name'. If the column isn't present, DNS-based
    matching is silently skipped - VM Name matching still works normally.
...
param(
    ...
    [string]$VCenterDnsNameColumn = 'DNS Name',
...
$hasDnsColumn = $vCenterColumns -contains $VCenterDnsNameColumn
if ($hasDnsColumn) {
    Write-Host "Using '$VCenterDnsNameColumn' from List A as a fallback match key (VM Name is tried first)."
} else {
    Write-Host "No '$VCenterDnsNameColumn' column found in List A - matching on VM Name only."
}

# Generic/placeholder hostnames that must never be used as a match key,
# since they are not unique to a single machine.
$genericHostnames = @('localhost', 'localhost.localdomain', 'unknown', 'n/a', '127.0.0.1')

# Count how often each (non-generic) DNS Name occurs across List A. Only
# DNS Names that occur exactly once are safe to use as a match key - a
# DNS Name shared by several VMs (e.g. a template default) can't reliably
# identify a single machine.
$dnsNameCounts = [System.Collections.Generic.Dictionary[string, int]]::new([StringComparer]::OrdinalIgnoreCase)
if ($hasDnsColumn) {
    foreach ($v in $vCenterVMs) {
        $dns = Get-ExactKey $v.$VCenterDnsNameColumn
        if ($dns -and ($genericHostnames -notcontains $dns.ToLowerInvariant())) {
            if ($dnsNameCounts.ContainsKey($dns)) { $dnsNameCounts[$dns]++ } else { $dnsNameCounts[$dns] = 1 }
        }
    }
}
...
foreach ($vmRow in $vCenterVMs) {
    ...
    # Build the list of candidate identifiers to try against Arc, in priority
    # order: VM Name first, then a unique/non-generic DNS Name as fallback.
    $candidates = [System.Collections.Generic.List[object]]::new()
    if ($exactKey) { $candidates.Add([pscustomobject]@{ Label = 'VM Name'; ExactKey = $exactKey; LooseKey = $looseKey }) }
    if ($hasDnsColumn) {
        $dnsExactKey = Get-ExactKey $vmRow.$VCenterDnsNameColumn
        if ($dnsExactKey -and ($genericHostnames -notcontains $dnsExactKey.ToLowerInvariant()) -and $dnsNameCounts.ContainsKey($dnsExactKey) -and $dnsNameCounts[$dnsExactKey] -eq 1) {
            $candidates.Add([pscustomobject]@{ Label = 'DNS Name'; ExactKey = $dnsExactKey; LooseKey = (Get-LooseKey $dnsExactKey) })
        }
    }
    ...
    if ($candidates.Count -eq 0) {
        # Blank/null VM name (and no usable DNS Name) - cannot be matched, handled safely
        $blankNameCount++
    }
    else {
        # 1) Try an exact match on each candidate, VM Name first
        $exactHit = $candidates | Where-Object { $arcExactIndex.ContainsKey($_.ExactKey) } | Select-Object -First 1

        if ($exactHit) {
            $exactMatchCount++
            if ($exactHit.Label -eq 'DNS Name') { $exactViaDnsCount++ }
            $onboardingStatus = 'Onboarded'
            $matchType        = 'Exact'
            $matchedVia       = "$($exactHit.Label): $($exactHit.ExactKey)"

            $matchedArcRows = $arcExactIndex[$exactHit.ExactKey]
            if ($resolvedArcStatusColumn) {
            ...
            # 2) No exact match on any candidate - check loose/possible match
            $notOnboardedCount++

            $looseHit = $candidates | Where-Object { $_.LooseKey -and $arcLooseIndex.ContainsKey($_.LooseKey) } | Select-Object -First 1
            if ($looseHit) {
                $possibleMatchCount++
                $matchType = 'Possible'
                $candidateNames = $arcLooseIndex[$looseHit.LooseKey] | ForEach-Object { $_.$ArcNameColumn } | Select-Object -Unique
...
Write-Host ("    - matched via DNS Name fallback   : {0}" -f $exactViaDnsCount)
