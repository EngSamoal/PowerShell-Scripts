...
.DESCRIPTION
    ...
    Matching logic:
      1. Exact match  : VM Name trimmed of leading/trailing whitespace, compared
                         CASE-SENSITIVELY. Only an exact match sets
      ...
      2. Possible match: Only evaluated for names that did NOT exact-match.
                         ...
                         for manual review only - it never sets Onboarded.
...
function Get-ExactKey {
    # Trim only. Case-sensitive on purpose - this is the "same string" bucket.
    param([string]$Name)
...
function Resolve-ArcStatusColumn {
    ...
    $candidates = @(
        'ArcAgentStatus', 'Agent Status', 'AgentStatus',
...
# Exact key -> list of Arc rows (case-sensitive, trimmed)
$arcExactIndex = [System.Collections.Generic.Dictionary[string, System.Collections.Generic.List[object]]]::new([StringComparer]::Ordinal)
# Loose key -> list of Arc rows (case/format-insensitive)
...
$vCenterNameSeen        = [System.Collections.Generic.Dictionary[string, int]]::new([StringComparer]::Ordinal)
