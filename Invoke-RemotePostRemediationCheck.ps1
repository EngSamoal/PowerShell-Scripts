...
Write-Host "Invoke-RemotePostRemediationCheck.ps1 - READ-ONLY verification  [build 2026-08-28h-mii-workgroup-na]" -ForegroundColor Magenta
...
# Domain membership - used to classify Machine Identity Isolation correctly on
# workgroup hosts. Default to $true if the query fails, so a real gap on a
# domain-joined box is never hidden.
$PartOfDomain = $true
...
try {
    ...
    if (-not $PartOfDomain) {
        # Machine Identity Isolation binds the Active Directory computer-account
        # credential (the domain secure-channel / Netlogon secret) into the VBS/IUM
        # enclave. A workgroup server has no AD machine identity to isolate, so the
        # policy is a no-op that does not persist - report it Not Applicable, not a gap.
        Add-Row -Header $H -Label 'Machine Identity Isolation' -LocalStatus 'OK' `
            -Detail 'N/A - workgroup (non-domain-joined) server has no Active Directory computer-account credential to isolate; the DeviceGuard\MachineIdentityIsolation policy is a no-op here and will not persist. Applicable only after the server is domain-joined.' `
            -Category 'VBS & Credential Protection' -Item 'Machine Identity Isolation' -NormStatus 'Not Applicable' `
            -Detected ("PartOfDomain=False; MachineIdentityIsolation={0}" -f $(if ($v.Found) { $v.Value } else { '(not set)' })) -Expected 'N/A unless domain-joined'
    } else {
