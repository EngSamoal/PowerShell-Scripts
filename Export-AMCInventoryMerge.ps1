...
$Sites = @(
    [pscustomobject]@{ Name = 'AMC'; VCenter = @('10.7.53.6'); Datacenter = @(); TabColor = 'BF8F00' }   # <-- AMC vCenter IP
)
...
function Add-InventorySheet([string]$Path, $Result, [string]$SheetName, [string]$TabColor, [string]$InsertBefore, [string]$Info) {
    ...
    try {
        ...
        $ct = Read-ZipXml $zip '[Content_Types].xml'; $cns = New-Ns $ct
        $old = @($sheets | Where-Object { $_.GetAttribute('name') -eq $SheetName })
        if ($old.Count -gt 0) {
            # Remove the existing tab (and its table) so it can be re-created with fresh data
            Write-Log "Replacing the existing '$SheetName' tab" 'WARN'
            $oldPos = [array]::IndexOf($sheets, $old[0])
            $oldRid = $old[0].GetAttribute('id', $NsRel)
            $oldRel = $wbRels.SelectSingleNode("/p:Relationships/p:Relationship[@Id='$oldRid']", $rns)
            $oldPart = Resolve-PartName 'xl' $oldRel.GetAttribute('Target')
            $drop = @($oldPart)
            $oldSheetRels = Read-ZipXml $zip (Get-RelsPartName $oldPart)
            if ($oldSheetRels) {
                $drop += Get-RelsPartName $oldPart
                foreach ($r in @($oldSheetRels.SelectNodes('/p:Relationships/p:Relationship', (New-Ns $oldSheetRels)))) {
                    if ($r.GetAttribute('Type') -like '*/table') { $drop += Resolve-PartName ($oldPart.Substring(0, $oldPart.LastIndexOf('/'))) $r.GetAttribute('Target') }
                }
            }
            foreach ($p in $drop) {
                $e = $zip.GetEntry($p); if ($e) { $e.Delete() }
                foreach ($o in @($ct.SelectNodes("/ct:Types/ct:Override[@PartName='/$p']", $cns))) { [void]$o.ParentNode.RemoveChild($o) }
            }
            [void]$oldRel.ParentNode.RemoveChild($oldRel)
            [void]$old[0].ParentNode.RemoveChild($old[0])
            foreach ($dn in @($wb.SelectNodes('/m:workbook/m:definedNames/m:definedName[@localSheetId]', $wns))) {
                $l = [int]$dn.GetAttribute('localSheetId')
                if ($l -eq $oldPos) { [void]$dn.ParentNode.RemoveChild($dn) }
                elseif ($l -gt $oldPos) { $dn.SetAttribute('localSheetId', [string]($l - 1)) }
            }
            foreach ($bv in @($wb.SelectNodes('/m:workbook/m:bookViews/m:workbookView', $wns))) {
                foreach ($a in 'activeTab', 'firstSheet') { if ($bv.HasAttribute($a) -and [int]$bv.GetAttribute($a) -gt $oldPos) { $bv.SetAttribute($a, [string]([int]$bv.GetAttribute($a) - 1)) } }
            }
            $sheets = @($wb.SelectNodes('/m:workbook/m:sheets/m:sheet', $wns))
        }
            ...
            if ($app) {
                ...
                for ($i = 0; $i -lt $vars.Count - 1; $i++) {
                    if ($vars[$i].InnerText -eq 'Worksheets') {
                        $cnt = $vars[$i + 1].SelectSingleNode('vt:i4', $ans)
                        $dup = @($lp | Select-Object -First ([int]$cnt.InnerText) | Where-Object { $_.InnerText -eq $SheetName })
                        foreach ($x in $dup) { [void]$parts.RemoveChild($x); $cnt.InnerText = [string]([int]$cnt.InnerText - 1) }
                        $lp = @($parts.SelectNodes('vt:lpstr', $ans))
                        $cnt.InnerText = [string]([int]$cnt.InnerText + 1)
...
if ($result.QueryOk -eq 0) {
    Write-Log "Nothing collected from AMC - the Excel file was NOT changed. Check the vCenter IP on line 15." 'ERROR'
    return
}
